...
class PostsMySqlExtDAO extends PostsMySqlDAO{
	...
	public function getPostToShare(){
		...
		$postsFetched = $this->getList($sqlQuery);
//print_r($postsFetched); exit;
		foreach ($postsFetched['id'] as $key => $value) {
			$sqlUpdate = "UPDATE posts SET share_count = 1 WHERE id = '$value' ";

			$sqlQuery = new SqlQuery($sqlUpdate);
			$sqlQuery->set($post->getShareCount());
			$id = $this->executeUpdate($sqlQuery);	
			$post -> setId($id);
			return $post;			
		}
